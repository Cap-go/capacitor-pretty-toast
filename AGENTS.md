...
# Build the plugin (TypeScript + Rollup)
bun run build
...
- `src/definitions.ts` - Public TypeScript types
- `src/controller.ts` - RN-style `toast.*` queueing and lifecycle logic
- `src/web.ts` / `src/web-renderer.ts` - Web bridge and DOM renderer
